// Array für die Posts
    ...
    const fallbackFeedUrl = 'https://api.allorigins.win/raw?url=' + encodeURIComponent('https://www.reddit.com/r/selfimprovement/.rss?limit=10');

    async function fetchFeed(url) {
      const response = await fetch(url, { cache: 'no-store' });
      if (!response.ok) throw new Error(`Fehler beim Laden des RSS-Feeds von ${url}`);
    ...
    async function loadPosts() {
      try {
        ...
        try {
          ...
          console.warn('Primärer RSS-Feed fehlgeschlagen, versuche Fallback:', primaryError);
        ...
        const posts = entries.map(entry => {
          const title = entry.querySelector('title')?.textContent?.trim() || 'Kein Titel';
          const link = entry.querySelector('link[rel="alternate"]')?.getAttribute('href') || entry.querySelector('link')?.textContent || '#';
        ...
        if (errorEl) {
          errorEl.textContent = `Fehler: ${error.message}. Stelle sicher, dass ./rss-proxy.php auf einem PHP-fähigen Server läuft oder der Fallback-Proxy verfügbar ist.`;
        }
